...
import com.sevendaystominecraft.client.ProgressionClientState;
...
public class ModNetworking {
    ...
    public static void onRegisterPayloads(RegisterPayloadHandlersEvent event) {
        ...
        registrar.playToClient(
                ...
                ModNetworking::handleProgressionSync
        );

        registrar.playToClient(
                SyncTraderDialoguePayload.TYPE,
                SyncTraderDialoguePayload.STREAM_CODEC,
                ModNetworking::handleTraderDialogueSync
        );

        registrar.playToServer(
                CrtTvActionPayload.TYPE,
                CrtTvActionPayload.STREAM_CODEC,
                ModNetworking::handleCrtTvAction
        );
    ...
    private static void handleProgressionSync(SyncProgressionPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            ProgressionClientState.update(payload.completedNodeIdSet(), payload.highestStage());
        });
    }

    private static void handleTraderDialogueSync(SyncTraderDialoguePayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            TraderDialogueClientState.update(payload.lines());
        });
    }
    ...
    private static void handleCrtTvAction(CrtTvActionPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            Player player = context.player();
            if (player == null) return;
            if (!(player instanceof ServerPlayer serverPlayer)) return;

            net.minecraft.core.BlockPos pos = new net.minecraft.core.BlockPos(payload.x(), payload.y(), payload.z());

            // Validate distance
            if (pos.distToCenterSqr(player.position()) > 64) return;

            net.minecraft.world.level.block.entity.BlockEntity be = serverPlayer.level().getBlockEntity(pos);
            if (!(be instanceof com.sevendaystominecraft.block.CrtTvBlockEntity tvEntity)) return;

            net.minecraft.world.level.block.state.BlockState state = serverPlayer.level().getBlockState(pos);

            String action = payload.action();
            if ("__off__".equals(action)) {
                tvEntity.stop();
                serverPlayer.level().setBlock(pos,
                        state.setValue(com.sevendaystominecraft.block.CrtTvBlock.POWERED, false), 3);
                com.sevendaystominecraft.sound.ModSounds.playAtBlock(
                        com.sevendaystominecraft.sound.ModSounds.CRT_POWER_OFF,
                        serverPlayer.level(), pos,
                        net.minecraft.sounds.SoundSource.BLOCKS, 0.8f, 1.0f);
            } else {
                tvEntity.setChannel(action);
                serverPlayer.level().setBlock(pos,
                        state.setValue(com.sevendaystominecraft.block.CrtTvBlock.POWERED, true), 3);
                com.sevendaystominecraft.sound.ModSounds.playAtBlock(
                        com.sevendaystominecraft.sound.ModSounds.CRT_POWER_ON,
                        serverPlayer.level(), pos,
                        net.minecraft.sounds.SoundSource.BLOCKS, 0.8f, 1.0f);
            }
        });
